...
from arlo_server.models import Interpretation
...
def run_audit_round(
    ...
    for i, ballot in enumerate(ballot_list):
        ...
        if i == 2:
            interpretation = Interpretation.CANT_AGREE
        if i == 3:
            interpretation = Interpretation.BLANK
        choice_id = candidate_id_1 if rand.random() > vote_ratio else candidate_id_2
        rv = post_json(
            client,
                ...
                "interpretations": [
                    {
                        ...
                        "comment": f"Comment for ballot {i}" if i % 3 == 0 else "",
                    }
                ]
            },
        )
...
def test_one_round_audit_report(client, election_id):
    ...
    expected = [
        ...
        # Round 1 Samples, ... a row containing all sampled ballots ...
        "All Sampled Ballots",
        "Ballot,Ticket Numbers,Audited?,Audit Result",
...
def test_two_round_audit_report(client, election_id):
    ...
    assert ballot_lines[:10] == [
        ...
        f'"Batch 10, #151",Round 1: 0.012381762,AUDITED,BLANK',
        f'"Batch 10, #175",Round 2: 0.021956866,AUDITED,{candidate_id_2}',
        f'"Batch 10, #200",Round 1: 0.000030407,AUDITED,{candidate_id_2}',
        f'"Batch 10, #59",Round 1: 0.002728647,AUDITED,{candidate_id_1}',
        f'"Batch 10, #72",Round 1: 0.009650515,AUDITED,CANT_AGREE',
