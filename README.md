...
- Server tests: `pipenv run pytest` (you can add flags - e.g. `-k <pattern>` only runs tests that match the pattern, `-n auto --dist loadfile` to run the tests in parallel)
