...
describe('Batch comparison data entry', () => {
  ...
  it('edits multiple tally sheets for a batch', async () => {
    ...
    const expectedCalls = [
      ...
      apiCalls.getBatches({
        ...batchesMocks.emptyInitial,
        batches: batchesWithResults([tallySheet1, tallySheet2, tallySheet3]),
      }),
      apiCalls.putBatchResults('batch-1', [
    ...
    await withMockFetch(expectedCalls, async () => {
      ...
      userEvent.click(screen.getByRole('button', { name: /Edit Tallies/ }))
      userEvent.click(
        screen.getByRole('button', { name: /Use Multiple Tally Sheets/ })
      )

      const voteTotalsTab = screen.getByRole('tab', { name: 'Vote Totals' })
      ...
      // Should get a validation error
      userEvent.click(screen.getByRole('button', { name: /Save Tallies/ }))
      await waitFor(() =>
        expect(choice2VotesInput).toHaveClass(Classes.INTENT_DANGER)
      )
      expect(choice2VotesInput).toHaveFocus()
      ...
      userEvent.click(await screen.findByText('Rename Sheet 3'))
      let newSheetNameInput = await screen.findByRole('textbox', {
        name: 'New Sheet Name',
      })
      userEvent.clear(newSheetNameInput)
      userEvent.type(newSheetNameInput, 'Sheet Three')
      userEvent.click(screen.getByRole('button', { name: 'Cancel' }))
      screen.getByRole('tab', { name: 'Sheet 3' })

      // Rename the sheet (save this second time)
      userEvent.click(
        screen.getByRole('button', { name: 'Additional Actions' })
      )
      userEvent.click(await screen.findByText('Rename Sheet 3'))
      newSheetNameInput = await screen.findByRole('textbox', {
        name: 'New Sheet Name',
      })
      userEvent.clear(newSheetNameInput)
      userEvent.type(newSheetNameInput, 'Sheet Three')
      userEvent.click(screen.getByRole('button', { name: 'Rename Sheet' }))
      await screen.findByRole('tab', { name: 'Sheet Three' })

      // Enter tallies for this new sheet
      batchTable = await screen.findByRole('table')
      rows = within(batchTable).getAllByRole('row')
      row1 = within(rows[1]).getAllByRole('cell')
      row2 = within(rows[2]).getAllByRole('cell')
      ...
      // Check that the vote totals tab displays sums before saving
      ...
      userEvent.click(await screen.findByText('Remove Sheet 1'))
      await waitFor(() =>
        expect(
      ...
      userEvent.click(await screen.findByText('Remove Sheet 2'))
      await waitFor(() =>
        expect(
