...
export interface Responders {
  add(responder: Responder): { remove(): void }
  addOneshot(responder: Responder): { remove(): void }
  mock(mock: MockRequestResponse): Responder
  oneshot(responder: Responder): Responder
}

export async function withFetchResponders(
  body: (responders: Responders) => Promise<void> | void
): Promise<void> {
  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (url, init) => {
    for (const responder of responders) {
      ...
      const response = await responder.respond(url.toString(), init)
  ...
  function oneshot(responder: Responder): Responder {
    let done = false
    return {
      async respond(url, init) {
        if (done) return undefined
        const response = await responder.respond(url, init)
        if (typeof response !== 'undefined') done = true
        return response
      },
    }
  }

  function fromMock(mock: MockRequestResponse): Responder {
    return {
      respond(url, init) {
        if (url !== mock.url) return undefined
        if (mock.options) {
          if (mock.options.method !== init?.method) return undefined
          if (mock.options.body !== init?.body) return undefined
          if (!equal(mock.options.headers, init?.headers)) return undefined
        }
        return new Response(JSON.stringify(mock.response), mock.error)
      },
    }
  }

  function add(responder: Responder): { remove(): void } {
  ...
  function addOneshot(responder: Responder): { remove(): void } {
    return add(oneshot(responder))
  }
